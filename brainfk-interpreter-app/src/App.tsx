...
function doBrainfuck(code: string, input: string = ''): string {
    ...
    let i = 0;
    let whileInits: Array<Array<number>> = [];
    ...
    for (i = 0; i < chars.length; i++) {
        ...
        switch (char) {
            ...
            case '<':
                ...
                break;
            case '+':
                stack[pointer] = (stack[pointer] || 0) + 1;
                break;
            case '-':
                stack[pointer] = (stack[pointer] || 0) - 1;
                break;
            case '.':
                output.push(String.fromCharCode(stack[pointer] || 0));
                break;
            case ',':
                stack[pointer] = inputCodeKeys[inputLoc++];
                break;
            case '[':
                whileInits.push([i, pointer]);
                break;
            case ']':
                if ((stack[pointer] || 0) === 0)
                    whileInits.pop();
                else
                    i = whileInits.at(-1)![0]; //;[whileInits.length-1][0];
